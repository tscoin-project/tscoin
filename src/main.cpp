...
#if defined(NDEBUG)
# error "Tscoin cannot be compiled without assertions."
#endif
...
uint256 hashGenesisBlock("0xac06369e83f521e2495093251c4fc7b9e82d722357f851be0eeca6d57c8accf1");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // Tscoin: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "Tscoin Signed Message:\n";
...
{
    // Tscoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    ...
    nSubsidy >>= (nHeight / 10000000); // Tscoin: 840k blocks in ~4 years

    return nSubsidy + nFees;
}

static const int64 nTargetTimespan = 3.5 * 24 * 60 * 60; // Tscoin: 3.5 days
static const int64 nTargetSpacing = 2.5 * 60; // Tscoin: 2.5 minutes
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // Tscoin: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Tscoin: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0x2c95bea705fb0c6b048fd7d272fd1f3e3ac291c63c97642693d065c87ab29075");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        block.nVersion = 1;
        block.nTime    = 1526387077;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 2085050178;

        if (fTestNet)
        {
            block.nTime    = 1526387062;
            block.nNonce   = 385558869;
        }
...
unsigned char pchMessageStart[4] = { 0xfa, 0xc3, 0xba, 0xd3 }; // Tscoin: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("TscoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("TscoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static TscoinMiner(CWallet *pwallet)
{
    printf("TscoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("tscoin-miner");
    ...
    try { loop {
        ...
        printf("Running TscoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&TscoinMiner, pwallet));
}
